  ...
  for (shock_var in var_names) {
    ...
    for (algo in colnames(residuals_var)[1:10]) {
      for (target_var in var_names) {
        ...
        shock = data.frame(residuals_var[algo])
        target = data.frame(oos_y[, target_var])
        ...
        #print(paste(shock_var, target_var))
...
png('Plots/ML/IRF_lag4_withCI.png', width = 1800, height = 1500, res = 200)
...
png('Plots/ML/IRF_lag4_noCI.png', width = 1800, height = 1500, res = 200)
irf_df_long %>% filter(algo %in% c('ARRF', 'FAARRF', 'AR', 'FAAR', 'RF tuned', 'GBM tuned'))  %>%
  ggplot(aes(x = index, y = mean, color = algo)) +
  geom_line() +
  geom_hline(aes(yintercept = 0), colour = "black", linetype = 'dashed') +
...
png('Plots/ML/IRF_VAR_withCI 2.png', width = 1800, height = 1500, res = 200)
...
png('Plots/ML/IRF_VAR_noCI 2.png', width = 1800, height = 1500, res = 200)
var_irf_df_long %>% filter(algo %in% c('ARRF', 'FAARRF', 'AR', 'FAAR', 'RF tuned', 'GBM tuned'))  %>%
  ggplot(aes(x = index, y = mean, color = algo)) +
  geom_line() +
  geom_hline(aes(yintercept = 0), colour = "black", linetype = 'dashed') +
